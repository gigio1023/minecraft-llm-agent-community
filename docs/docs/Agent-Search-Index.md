...
| `ACTION_SURFACE` | Provider-visible direct/deferred affordance packet for the current actor body; not a domain strategy or home-building checklist | `Specification/Runtime-Evidence-And-Action-Skills.md`, `SPEC.md`, `Architecture/Future-Works.md`, `probe/src/runtime/actionSurface.ts` |
...
| `REAL_SERVER_SIMULATION_TEST_PLAN` | Live-server simulation test protocol for connected Mineflayer actors, explicit live provider social cycles, action-skill evidence, provider usage guard, and coal/shelter readiness gates | `Architecture/Real-Server-Simulation-Test-Plan.md`, `Setup/Headless-Server.md`, `Setup/Provider-Setup.md` |
| `FUTURE_WORKS` | Live-run follow-ups that do not change the long-term spec: 100-cycle home-base findings, blocked-retry pivot rules, partial-progress reporting, review summary catch-up, and Minecraft MCP/Codex action-interface reference adaptation | `Architecture/Future-Works.md`, `Architecture/Real-Server-Simulation-Test-Plan.md`, `Architecture/Current-Handoff-And-Next-Work.md` |
