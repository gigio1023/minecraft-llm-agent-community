...
2026-05-31 Beads alignment 이후 새 persistent middle layer는
`PlanBeadGraph`다. 이 문서의 `StrategicGoal`은 medium-horizon interpretation을
설명하는 legacy-adjacent 개념으로 읽어야 한다. 새 구현에서 restart-safe
living work state가 필요하면 `StrategicGoal` store를 새로 늘리지 말고
PlanBead issue-like records와 dependency edges로 저장한다. `StrategicGoal`은
PlanBead summary/projection 또는 migration input이 될 수 있지만 두 번째 active
middle layer가 되면 안 된다.
...
flowchart TB
  ...
  LifeGoal --> CycleGoalProvider["LLM CycleGoal Provider<br/>select ready PlanBead context and CycleGoal"]
  ...
  Beads["PlanBeadGraph<br/>actor-owned issue/work graph"] --> Context
  ...
  CycleGoalProvider --> PlanBead["ready PlanBead selection<br/>read-only work context"]
...
flowchart TB
  ...
  B --> C1["Open PlanBead<br/>stabilize shared storage"]
  B --> C2["Open PlanBead<br/>improve dangerous resource capability"]
  B --> C3["Open PlanBead<br/>fulfill open social obligations"]
  C2 --> D["Ready PlanBead<br/>prepare diamond acquisition because the settlement can now use higher-tier tools"]
...
PlanBeadGraph 안의 ready PlanBead다. Older docs may call the same
medium-horizon interpretation a `StrategicGoal`, but new restart-safe living
work state should use PlanBeads and dependency edges.
