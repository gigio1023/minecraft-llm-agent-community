...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%Track Changes Section. Amos Storkey                           %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%EDIT%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newif\ifincludecomment
\includecommenttrue % \includecommenttrue or \includecommentflase
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\ifincludecomment
...
%https://v2.overleaf.com/project/5bc654d17becd14dc7d9222a% as long as the \iclrfinalcopy macro remains commented out below.
...
\label{fig:spikes}
\end{figure}
% pythonw -m rl_algs.ppo_metal.replayer --tag master_cnn_0c7d9762_MontezumaRevenge_128env_025exp_gammaext0999_seed4_180905 --filter rew --rew_min 10200
...
\label{sec:rnd}
%Describe in detail common approach.
%Give the alternative (error of a prediction problem), and say how it is different.
%Given your proposed approach, clarify your claims about it, and set it in context of the current work: how is it different.
...
It is not obvious how to estimate the combined value of the non-episodic stream of intrinsic rewards $i_t$ and the episodic stream of extrinsic rewards $e_t$. Our solution is to observe that the return is linear in the rewards and so can be decomposed as a sum $R = R_E + R_I$ of the extrinsic and intrinsic returns respectively. Hence we can fit two value heads $V_E$ and $V_I$ separately using their respective returns, and combine them to give the value function $V = V_E + V_I$. This same idea can also be used to combine reward streams with different discount factors.% In most of our experiments we used a discount factor of $\gamma_I = 0.99$ for the intrinsic reward and $\gamma_E = 0.999$ for the extrinsic reward.
...
% prediction methods (deepak, us)
% improvement prediction
% demonstration on MZ, # exploration (special access to MZ), Jeff Clune, Josh Tenenbaum
% Miscellaneous exploration methods - adversarial self-play, other weird pseudo-counts, feudal networks, parameter noise, noisy networks, variational intrinsic control, diversity is all you need, variational option discovery, Ian Osband paper (randomized value functions), Ensembles and Thompson sampling, Dora the explorer, learning awareness models, count-based exploration with successor representation, novelty search
% Random features 
% Novelty detection
