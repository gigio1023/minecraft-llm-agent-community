\section{Instructions for \sotopiaeval}
\label{app:eval_instructions}
We use the the same set of instructions for both GPT-4 \citep{openai2023gpt4} and humans when evaluating episodes in \sotopia. In these instructions, we provide explanation for each dimension, the requirements for reasoning, and the ranges of scores one could give. 
