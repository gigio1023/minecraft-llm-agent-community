# Successor Agent Prompt: Live Work After A3/B2

You are continuing the V4 Minecraft social-sandbox implementation after the
provider-free A3/B2 successor repairs and the Codex review in `63eb47df`.
Treat their acceptance as provider-free only; live provider and restart behavior
...
- A3 includes server/world preparation in wall time and records
  `budget_stopped` separately from target-miss `budget_exhausted`. Request,
  token, and cost checks prevent further work after an observed limit.
- B2 deeply validates `goal-continuity-artifact-bag/v1`, rejects symbolic-link
  traversal below the declared root, asserts bags at evaluate entry, and can
  write typed restart observations offline.
...
- Key commits: `76db6cc0` (B2 loader), `75a68ec9` (A3 stops), `83b26f85`
  (B2 evaluate assert), `c4266841` (A3 labeling/wall observe), `63eb47df`
  (Codex review repairs)
- Branch state: ahead of origin and not pushed unless the user asked
- Validation snapshot: `cd probe && bun test` → 729 pass; typecheck pass;
...
Apply default `full` Ponytail discipline through the adaptation in the active
implementation plan: remove needless code, but never remove validation, saved
evidence, provider limits, or focused failure tests.
...
- Provider HTTP calls do not receive the case AbortSignal once a request is in
  flight. The first approved A5 smoke must test and record this behavior.
