...
For author_mineflayer_action, source must define export async function run(ctx, params) with no import, require, process, filesystem, network, eval, Function, while(true), or for(;;). Use only direct ctx helpers named in helper_allowlist, for example ctx.observe(...), ctx.wait(...), or ctx.mineflayer.lookAtNearestBlock(...). Do not use ctx.helpers, ctx.sharedStorage, ctx.bot, or ctx.mineflayer() as a function. Supported helper names are exactly: position, inventoryItems, observe, wait, collectLogs, mineBlock, craftItem, craftWithTable, consumeItem, placeBlock, buildPattern, say, mineflayer. The mineflayer helper exposes only these methods: lookAt, lookAtNearestBlock, setControlState, clearControlStates, swingArm, equipByName, activateItem, deactivateItem, chat.
